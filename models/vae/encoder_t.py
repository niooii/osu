...
class ReplayEncoderT(nn.Module):
    def __init__(
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=self.transformer_args.embed_dim, nhead=self.transformer_args.attn_heads, dim_feedforward=self.transformer_args.ff_dim, batch_first=True)

        self.encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=self.transformer_args.transformer_layers)

        self.mu_layer = nn.Sequential(
            nn.Linear(in_features=self.transformer_args.embed_dim, out_features=latent_dim * 2),
            nn.ReLU(),
